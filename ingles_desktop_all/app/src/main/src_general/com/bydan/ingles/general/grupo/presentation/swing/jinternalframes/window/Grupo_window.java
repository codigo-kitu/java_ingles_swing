...
*/
package com.bydan.ingles.general.grupo.presentation.swing.jinternalframes.window;
...
public class Grupo_window extends JInternalFrameBase {	
	...
	public Grupo_form_window jFrameDetalleForm;//_form;		
	...
	*/
	...
	public Grupo_window() throws Exception {
		super(PaginaTipo.PRINCIPAL);
        //super("Grupo No " + (++openFrameCount),true, /*resizable*/true, /*closable*/true, /*maximizable*/true);//iconifiable
	  	try {
	...
	public Grupo_window(Boolean cargarRelaciones,PaginaTipo paginaTipo) throws Exception {
		super(paginaTipo);
        //super("Grupo No " + (++openFrameCount),true, /*resizable*/true, /*closable*/true, /*maximizable*/true);//iconifiable
	  	try {
	...
	public Grupo_window(Boolean conGuardarRelaciones,Boolean esGuardarRelacionado,Boolean cargarRelaciones,PaginaTipo paginaTipo) throws Exception {
		super(paginaTipo);
        //super("Grupo No " + (++openFrameCount),true, /*resizable*/true, /*closable*/true, /*maximizable*/true);//iconifiable
	  	try {
	...
	public List<Grupo> getgruposAux() {
		return this.gruposAux;
	}

	public void setgruposAux(List<Grupo> gruposAux) {
		this.gruposAux = gruposAux;
	}
	
	public List<Grupo> getgruposEliminados() {
		return this.gruposEliminados;
	}

	public void setGruposEliminados(List<Grupo> gruposEliminados) {
	...
	//		JCheckBox jCheckBoxConGraficoDinamicoGrupo) {
	//	this.jCheckBoxConGraficoDinamicoGrupo = jCheckBoxConGraficoDinamico;
	//}	
	
	//public void setBorderResaltarConGraficoDinamico() {
	//	Border borderResaltar=Funciones2.getBorderResaltar(this.getParametroGeneralUsuario(),"PARAMETRO");
	//	this.jPanelToolBar.toolBar.setBorder(borderResaltar);
	//	this.jCheckBoxConGraficoDinamicoGrupo.setBorder(borderResaltar);		
	//}
	...
	*/
	...
	private void initialize(JDesktopPane jdesktopPane,Boolean conGuardarRelaciones,Boolean esGuardarRelacionado,Boolean cargarRelaciones,Usuario usuarioActual,ResumenUsuario resumenUsuarioActual,Modulo moduloActual,Opcion opcionActual,ParametroGeneralSg parametroGeneralSg,ParametroGeneralUsuario parametroGeneralUsuario,PaginaTipo paginaTipo) throws Exception {
		...
		this.conCargarMinimo=this.grupo_session.getEsGuardarRelacionado();				
		...
		//this.sTipoTamanioGeneral=Grupo_window.S_TIPO_TAMANIO_GENERAL;
		//this.sTipoTamanioGeneralTabla=Grupo_window.S_TIPO_TAMANIO_GENERAL_TABLA;
		...
		Grupo_window.S_TIPO_TAMANIO_GENERAL=this.sTipoTamanioGeneral;
		Grupo_window.S_TIPO_TAMANIO_GENERAL_TABLA=this.sTipoTamanioGeneralTabla;
		Grupo_window.CONTIPO_TAMANIO_MANUAL=this.conTipoTamanioManual;
		...
		//this.setTitle(Funciones.GetTituloSistema(this.parametroGeneralSg,this.moduloActual,this.opcionActual,this.usuarioActual,"Grupo MANTENIMIENTO"));
		...
		Grupo_window.ESTA_CARGADO_PORPARTE=true;
	...
	public JScrollPane getJContentPane(int iWidth,int iHeight,JDesktopPane jDesktopPane,Boolean cargarRelaciones,Usuario usuarioActual,ResumenUsuario resumenUsuarioActual,Modulo moduloActual,Opcion opcionActual,ParametroGeneralSg parametroGeneralSg,ParametroGeneralUsuario parametroGeneralUsuario,PaginaTipo paginaTipo) throws Exception {	
		...
		if(this.conCargarFormDetalle) { //true) {
			//this.jFrameDetalle = new GrupoDetalle_window(paginaTipo);//JInternalFrameBase("Grupo DATOS");
			//_form
			this.jFrameDetalleForm = new Grupo_form_window(jDesktopPane,this.grupo_session.getConGuardarRelaciones(),this.grupo_session.getEsGuardarRelacionado(),cargarRelaciones,usuarioActual,resumenUsuarioActual,moduloActual,opcionActual,parametroGeneralSg,parametroGeneralUsuario,paginaTipo);
		} else {
			//_form
			this.jFrameDetalleForm = null;//new Grupo_form_window("MINIMO");
		}
		...
		this.gridaBagLayout= new GridBagLayout();//Grupo
		
		
		this.tableDatos = new JTable();      
		
		String sToolTipGrupo="";
		sToolTipGrupo=Grupo_util.S_CLASS_WEB_TITULO;
		
		if(Constantes.IS_DEVELOPING) {
			sToolTipGrupo+="(General.Grupo)";
		}
		
		if(!this.grupo_session.getEsGuardarRelacionado()) {
			sToolTipGrupo+="_"+this.opcionActual.getId();
		}
		
		this.tableDatos.setToolTipText(sToolTipGrupo);
		...
		this.sPath=" <---> Acceso: Grupo";
		
		if(!this.grupo_session.getEsGuardarRelacionado()) {
			this.jScrollPanelDatos.setBorder(javax.swing.BorderFactory.createTitledBorder("Datos Grupos" + this.sPath));
		} else {
		...
		if(!this.conCargarMinimo) {
			...
			//this.jFrameOrderBy=new OrderByJInternalFrame(S_TIPO_TAMANIO_GENERAL,this.jButtonAbrirOrderByGrupo,false,this);
			
			//this.cargarOrderBy(false);
		} else {
			//NO CARGAR INICIALMENTE, EN BOTON AL ABRIR
			//this.jFrameOrderBy=new OrderByJInternalFrame(S_TIPO_TAMANIO_GENERAL,this.jButtonAbrirOrderByGrupo,true,this);
		...
		*/
		...
		this.jPanelParametrosReportes = new JPanelParametrosReportes(this,"Grupo",this.parametroGeneralUsuario.getid_tipo_fondo(),S_TIPO_TAMANIO_GENERAL,this.conCargarMinimo,false,false,true);//new JPanel();
		...
		this.jPanelPaginacion = new JPanelPaginacion(this,this.parametroGeneralUsuario.getid_tipo_fondo(),true,this.conCargarMinimo,this.grupo_session.getEsGuardarRelacionado(),S_TIPO_TAMANIO_GENERAL,"Grupo",PaginaTipo.PRINCIPAL,false,this.conFuncionalidadRelaciones,false);
		this.jPanelToolBar = new JPanelToolBar(this.parametroGeneralUsuario.getid_tipo_fondo(),"Grupo",PaginaTipo.PRINCIPAL,this);
		...
		//FuncionesSwing.addImagenButtonGeneral(this.jButtonRecargarInformacion,"recargar_button","Recargar","Recargar"+" "+Grupo_util.S_CLASS_WEB_TITULO + FuncionesSwing.getKeyMensaje("RECARGAR"));		
		
		
		
		
		this.btn_ProcesarInformacion = new JButton();
		FuncionesSwing.setButtonProp(this.btn_ProcesarInformacion,"Procesar","Procesar"+" "+Grupo_util.S_CLASS_WEB_TITULO,false);
		...
		*/
		...
		//ESTA EN BEAN
		/*
		this.jTabbedPaneRelaciones.setMinimumSize(new Dimension(this.getWidth(),GrupoBean.ALTO_TABPANE_RELACIONES  + FuncionesSwing.getValorProporcion(GrupoBean.ALTO_TABPANE_RELACIONES,0)));
		this.jTabbedPaneRelaciones.setMaximumSize(new Dimension(this.getWidth(),GrupoBean.ALTO_TABPANE_RELACIONES  + FuncionesSwing.getValorProporcion(GrupoBean.ALTO_TABPANE_RELACIONES,0)));
		this.jTabbedPaneRelaciones.setPreferredSize(new Dimension(this.getWidth(),GrupoBean.ALTO_TABPANE_RELACIONES  + FuncionesSwing.getValorProporcion(GrupoBean.ALTO_TABPANE_RELACIONES,0)));
		*/
		...
		/*
			GridBagLayout gridaBagLayoutDatosGrupo = new GridBagLayout();

			this.jScrollPanelDatos.setLayout(gridaBagLayoutDatosGrupo);
		...
		*/
		...
		this.jPanelAcciones.add(this.jButtonNuevoGrupo, this.gridBagConstraints);
		*/
		int iPosXAccion=0;
...
if(this.parametroGeneralUsuario.getcon_botones_tool_bar() && !this.grupo_session.getEsGuardarRelacionado()) {			
	...
	public void redimensionarTablaDatos(int iNumFilas,int iTamanioFilaTabla) {
		...
		if(this.conMaximoRelaciones 
			&& this.grupo_session.getConGuardarRelaciones()
			) {
		...
		*/
		...
		if(iTamanioFilaTabla<=0) {
			iTamanioFilaTabla=this.tableDatos.getRowHeight();//Grupo_util.ITAMANIOFILATABLA;
		}
		...
		if(!this.grupo_session.getEsGuardarRelacionado()) {			
			if(iHeightTable > Grupo_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS) { //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS) {
				//SI SE SELECCIONA MAXIMO TABLA SE AMPLIA A ALTO MAXIMO DE SCROLL, PARA QUE SCROLL NO SEA TAN PEQUE?O
				
				if(!this.jPanelParametrosReportes.chb_ConAltoMaximoTabla.isSelected()) {
					iHeightTable=Grupo_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS; //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS;
				} else {
				
					iHeightTable=iHeightTableTotal + FuncionesSwing.getValorProporcion(iHeightTableTotal,30);
				}
			} else {
				if(iHeightTable < Grupo_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS) {//Constantes.ISWING_TAMANIOMINIMO_TABLADATOS) {
					iHeightTable=Grupo_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS; //Constantes.ISWING_TAMANIOMINIMO_TABLADATOS;
				}
			}
			
		} else {
			if(iHeightTable > Grupo_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS_REL) { //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS) {
				//SI SE SELECCIONA MAXIMO TABLA SE AMPLIA A ALTO MAXIMO DE SCROLL, PARA QUE SCROLL NO SEA TAN PEQUE?O
				
				if(!this.jPanelParametrosReportes.chb_ConAltoMaximoTabla.isSelected()) {
					iHeightTable=Grupo_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS_REL; //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS;
				} else {
				
					iHeightTable=iHeightTableTotal + FuncionesSwing.getValorProporcion(iHeightTableTotal,30);
				}
			} else {
				if(iHeightTable < Grupo_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS_REL) {//Constantes.ISWING_TAMANIOMINIMO_TABLADATOS) {
					iHeightTable=Grupo_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS_REL; //Constantes.ISWING_TAMANIOMINIMO_TABLADATOS;
				}
			}
		}
	...
	//PARA REPORTES
	public static List<Grupo> TraerBeans(List<Grupo> grupos,ArrayList<Classe> classes)throws Exception {
		try {
			for(Grupo grupo:grupos) {
					
				if(!(Grupo_util.S_TIPO_REPORTE_EXTRA.equals(Constantes2.S_REPORTE_EXTRA_GROUP_GENERICO)
						  || Grupo_util.S_TIPO_REPORTE_EXTRA.equals(Constantes2.S_REPORTE_EXTRA_GROUP_TOTALES_GENERICO)) 
				   ) {
					
					grupo.setsDetalleGeneralEntityReporte(Grupo_util.getGrupoDescripcion(grupo));
					...
					//grupo.setsDetalleGeneralEntityReporte(grupo.getsDetalleGeneralEntityReporte());
										
				}
				
				//grupobeans.add(grupobean);
			}
		} catch(Exception ex) {
			throw ex;
		}
	...
	/*
		public interface Grupo_windowI {	
