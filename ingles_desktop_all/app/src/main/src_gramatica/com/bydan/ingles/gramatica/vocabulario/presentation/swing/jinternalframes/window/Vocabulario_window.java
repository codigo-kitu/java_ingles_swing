...
*/
package com.bydan.ingles.gramatica.vocabulario.presentation.swing.jinternalframes.window;
...
public class Vocabulario_window extends JInternalFrameBase {	
	...
	public Vocabulario_form_window jFrameDetalleForm;//_form;		
	...
	*/
	...
	public Vocabulario_window() throws Exception {
		super(PaginaTipo.PRINCIPAL);
        //super("Vocabulario No " + (++openFrameCount),true, /*resizable*/true, /*closable*/true, /*maximizable*/true);//iconifiable
	  	try {
	...
	public Vocabulario_window(Boolean cargarRelaciones,PaginaTipo paginaTipo) throws Exception {
		super(paginaTipo);
        //super("Vocabulario No " + (++openFrameCount),true, /*resizable*/true, /*closable*/true, /*maximizable*/true);//iconifiable
	  	try {
	...
	public Vocabulario_window(Boolean conGuardarRelaciones,Boolean esGuardarRelacionado,Boolean cargarRelaciones,PaginaTipo paginaTipo) throws Exception {
		super(paginaTipo);
        //super("Vocabulario No " + (++openFrameCount),true, /*resizable*/true, /*closable*/true, /*maximizable*/true);//iconifiable
	  	try {
	...
	public List<Vocabulario> getvocabulariosAux() {
		return this.vocabulariosAux;
	}

	public void setvocabulariosAux(List<Vocabulario> vocabulariosAux) {
		this.vocabulariosAux = vocabulariosAux;
	}
	
	public List<Vocabulario> getvocabulariosEliminados() {
		return this.vocabulariosEliminados;
	}

	public void setVocabulariosEliminados(List<Vocabulario> vocabulariosEliminados) {
		this.vocabulariosEliminados = vocabulariosEliminados;
	}
	...
	//		JCheckBox jCheckBoxConGraficoDinamicoVocabulario) {
	//	this.jCheckBoxConGraficoDinamicoVocabulario = jCheckBoxConGraficoDinamico;
	//}	
	
	//public void setBorderResaltarConGraficoDinamico() {
	//	Border borderResaltar=Funciones2.getBorderResaltar(this.getParametroGeneralUsuario(),"PARAMETRO");
	//	this.jPanelToolBar.toolBar.setBorder(borderResaltar);
	//	this.jCheckBoxConGraficoDinamicoVocabulario.setBorder(borderResaltar);		
	//}
	...
	*/
	...
	private void initialize(JDesktopPane jdesktopPane,Boolean conGuardarRelaciones,Boolean esGuardarRelacionado,Boolean cargarRelaciones,Usuario usuarioActual,ResumenUsuario resumenUsuarioActual,Modulo moduloActual,Opcion opcionActual,ParametroGeneralSg parametroGeneralSg,ParametroGeneralUsuario parametroGeneralUsuario,PaginaTipo paginaTipo) throws Exception {
		...
		this.vocabulario_session.setConGuardarRelaciones(conGuardarRelaciones);
		this.vocabulario_session.setEsGuardarRelacionado(esGuardarRelacionado);
		
		
		this.conCargarMinimo=this.vocabulario_session.getEsGuardarRelacionado();				
		...
		//this.sTipoTamanioGeneral=Vocabulario_window.S_TIPO_TAMANIO_GENERAL;
		//this.sTipoTamanioGeneralTabla=Vocabulario_window.S_TIPO_TAMANIO_GENERAL_TABLA;
		...
		Vocabulario_window.S_TIPO_TAMANIO_GENERAL=this.sTipoTamanioGeneral;
		Vocabulario_window.S_TIPO_TAMANIO_GENERAL_TABLA=this.sTipoTamanioGeneralTabla;
		Vocabulario_window.CONTIPO_TAMANIO_MANUAL=this.conTipoTamanioManual;
		...
		//this.setTitle(Funciones.GetTituloSistema(this.parametroGeneralSg,this.moduloActual,this.opcionActual,this.usuarioActual,"Vocabulario MANTENIMIENTO"));
		...
		Vocabulario_window.ESTA_CARGADO_PORPARTE=true;
	...
	public JScrollPane getJContentPane(int iWidth,int iHeight,JDesktopPane jDesktopPane,Boolean cargarRelaciones,Usuario usuarioActual,ResumenUsuario resumenUsuarioActual,Modulo moduloActual,Opcion opcionActual,ParametroGeneralSg parametroGeneralSg,ParametroGeneralUsuario parametroGeneralUsuario,PaginaTipo paginaTipo) throws Exception {	
		...
		if(this.conCargarFormDetalle) { //true) {
			//this.jFrameDetalle = new VocabularioDetalle_window(paginaTipo);//JInternalFrameBase("Vocabulario DATOS");
			//_form
			this.jFrameDetalleForm = new Vocabulario_form_window(jDesktopPane,this.vocabulario_session.getConGuardarRelaciones(),this.vocabulario_session.getEsGuardarRelacionado(),cargarRelaciones,usuarioActual,resumenUsuarioActual,moduloActual,opcionActual,parametroGeneralSg,parametroGeneralUsuario,paginaTipo);
		} else {
			//_form
			this.jFrameDetalleForm = null;//new Vocabulario_form_window("MINIMO");
		}
		...
		this.gridaBagLayout= new GridBagLayout();//Vocabulario
		
		
		this.tableDatos = new JTable();      
		
		String sToolTipVocabulario="";
		sToolTipVocabulario=Vocabulario_util.S_CLASS_WEB_TITULO;
		
		if(Constantes.IS_DEVELOPING) {
			sToolTipVocabulario+="(Gramatica.Vocabulario)";
		}
		
		if(!this.vocabulario_session.getEsGuardarRelacionado()) {
			sToolTipVocabulario+="_"+this.opcionActual.getId();
		}
		
		this.tableDatos.setToolTipText(sToolTipVocabulario);
		...
		this.sPath=" <---> Acceso: Vocabulario";
		
		if(!this.vocabulario_session.getEsGuardarRelacionado()) {
			this.jScrollPanelDatos.setBorder(javax.swing.BorderFactory.createTitledBorder("Datos Vocabularios" + this.sPath));
		} else {
		...
		if(!this.conCargarMinimo) {
			...
			//this.jFrameOrderBy=new OrderByJInternalFrame(S_TIPO_TAMANIO_GENERAL,this.jButtonAbrirOrderByVocabulario,false,this);
			
			//this.cargarOrderBy(false);
		} else {
			//NO CARGAR INICIALMENTE, EN BOTON AL ABRIR
			//this.jFrameOrderBy=new OrderByJInternalFrame(S_TIPO_TAMANIO_GENERAL,this.jButtonAbrirOrderByVocabulario,true,this);
		...
		*/
		...
		this.jPanelParametrosReportes = new JPanelParametrosReportes(this,"Vocabulario",this.parametroGeneralUsuario.getid_tipo_fondo(),S_TIPO_TAMANIO_GENERAL,this.conCargarMinimo,false,false,true);//new JPanel();
		...
		this.jPanelPaginacion = new JPanelPaginacion(this,this.parametroGeneralUsuario.getid_tipo_fondo(),true,this.conCargarMinimo,this.vocabulario_session.getEsGuardarRelacionado(),S_TIPO_TAMANIO_GENERAL,"Vocabulario",PaginaTipo.PRINCIPAL,false,this.conFuncionalidadRelaciones,false);
		this.jPanelToolBar = new JPanelToolBar(this.parametroGeneralUsuario.getid_tipo_fondo(),"Vocabulario",PaginaTipo.PRINCIPAL,this);
		...
		//FuncionesSwing.addImagenButtonGeneral(this.jButtonRecargarInformacion,"recargar_button","Recargar","Recargar"+" "+Vocabulario_util.S_CLASS_WEB_TITULO + FuncionesSwing.getKeyMensaje("RECARGAR"));		
		
		
		
		
		this.btn_ProcesarInformacion = new JButton();
		FuncionesSwing.setButtonProp(this.btn_ProcesarInformacion,"Procesar","Procesar"+" "+Vocabulario_util.S_CLASS_WEB_TITULO,false);
		...
		*/
		...
		//ESTA EN BEAN
		/*
		this.jTabbedPaneRelaciones.setMinimumSize(new Dimension(this.getWidth(),VocabularioBean.ALTO_TABPANE_RELACIONES  + FuncionesSwing.getValorProporcion(VocabularioBean.ALTO_TABPANE_RELACIONES,0)));
		this.jTabbedPaneRelaciones.setMaximumSize(new Dimension(this.getWidth(),VocabularioBean.ALTO_TABPANE_RELACIONES  + FuncionesSwing.getValorProporcion(VocabularioBean.ALTO_TABPANE_RELACIONES,0)));
		this.jTabbedPaneRelaciones.setPreferredSize(new Dimension(this.getWidth(),VocabularioBean.ALTO_TABPANE_RELACIONES  + FuncionesSwing.getValorProporcion(VocabularioBean.ALTO_TABPANE_RELACIONES,0)));
		*/
		...
		/*
			GridBagLayout gridaBagLayoutDatosVocabulario = new GridBagLayout();

			this.jScrollPanelDatos.setLayout(gridaBagLayoutDatosVocabulario);
		...
		*/
		...
		this.jPanelAcciones.add(this.jButtonNuevoVocabulario, this.gridBagConstraints);
		*/
		int iPosXAccion=0;
		...
		GridBagLayout gridaBagLayoutBusquedaPorNombreVocabulario= new GridBagLayout();
		gridaBagLayoutBusquedaPorNombreVocabulario.rowHeights = new int[] {1};
		gridaBagLayoutBusquedaPorNombreVocabulario.columnWidths = new int[] {1};
		gridaBagLayoutBusquedaPorNombreVocabulario.rowWeights = new double[]{0.0, 0.0, 0.0};
		gridaBagLayoutBusquedaPorNombreVocabulario.columnWeights = new double[]{0.0, 1.0};
		pnl_BusquedaPorNombre.setLayout(gridaBagLayoutBusquedaPorNombreVocabulario);
		...
		GridBagLayout gridaBagLayoutBusquedaPorTraduccionVocabulario= new GridBagLayout();
		gridaBagLayoutBusquedaPorTraduccionVocabulario.rowHeights = new int[] {1};
		gridaBagLayoutBusquedaPorTraduccionVocabulario.columnWidths = new int[] {1};
		gridaBagLayoutBusquedaPorTraduccionVocabulario.rowWeights = new double[]{0.0, 0.0, 0.0};
		gridaBagLayoutBusquedaPorTraduccionVocabulario.columnWeights = new double[]{0.0, 1.0};
		pnl_BusquedaPorTraduccion.setLayout(gridaBagLayoutBusquedaPorTraduccionVocabulario);
		...
		GridBagLayout gridaBagLayoutFK_IdAreaVocabulario= new GridBagLayout();
		gridaBagLayoutFK_IdAreaVocabulario.rowHeights = new int[] {1};
		gridaBagLayoutFK_IdAreaVocabulario.columnWidths = new int[] {1};
		gridaBagLayoutFK_IdAreaVocabulario.rowWeights = new double[]{0.0, 0.0, 0.0};
		gridaBagLayoutFK_IdAreaVocabulario.columnWeights = new double[]{0.0, 1.0};
		pnl_FK_IdArea.setLayout(gridaBagLayoutFK_IdAreaVocabulario);
		...
		GridBagLayout gridaBagLayoutFK_IdNivelVocabulario= new GridBagLayout();
		gridaBagLayoutFK_IdNivelVocabulario.rowHeights = new int[] {1};
		gridaBagLayoutFK_IdNivelVocabulario.columnWidths = new int[] {1};
		gridaBagLayoutFK_IdNivelVocabulario.rowWeights = new double[]{0.0, 0.0, 0.0};
		gridaBagLayoutFK_IdNivelVocabulario.columnWeights = new double[]{0.0, 1.0};
		pnl_FK_IdNivel.setLayout(gridaBagLayoutFK_IdNivelVocabulario);
		...
		GridBagLayout gridaBagLayoutFK_IdSubAreaVocabulario= new GridBagLayout();
		gridaBagLayoutFK_IdSubAreaVocabulario.rowHeights = new int[] {1};
		gridaBagLayoutFK_IdSubAreaVocabulario.columnWidths = new int[] {1};
		gridaBagLayoutFK_IdSubAreaVocabulario.rowWeights = new double[]{0.0, 0.0, 0.0};
		gridaBagLayoutFK_IdSubAreaVocabulario.columnWeights = new double[]{0.0, 1.0};
		pnl_FK_IdSubArea.setLayout(gridaBagLayoutFK_IdSubAreaVocabulario);
...
if(this.parametroGeneralUsuario.getcon_botones_tool_bar() && !this.vocabulario_session.getEsGuardarRelacionado()) {			
		...
		*/		
		...
		if(Vocabulario_window.CON_DATOS_FRAME) {				
	...
	public void redimensionarTablaDatos(int iNumFilas,int iTamanioFilaTabla) {
		...
		if(this.conMaximoRelaciones 
			&& this.vocabulario_session.getConGuardarRelaciones()
			) {
		...
		*/
		...
		if(iTamanioFilaTabla<=0) {
			iTamanioFilaTabla=this.tableDatos.getRowHeight();//Vocabulario_util.ITAMANIOFILATABLA;
		}
		...
		if(!this.vocabulario_session.getEsGuardarRelacionado()) {			
			if(iHeightTable > Vocabulario_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS) { //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS) {
				//SI SE SELECCIONA MAXIMO TABLA SE AMPLIA A ALTO MAXIMO DE SCROLL, PARA QUE SCROLL NO SEA TAN PEQUE?O
				
				if(!this.jPanelParametrosReportes.chb_ConAltoMaximoTabla.isSelected()) {
					iHeightTable=Vocabulario_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS; //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS;
				} else {
				
					iHeightTable=iHeightTableTotal + FuncionesSwing.getValorProporcion(iHeightTableTotal,30);
				}
			} else {
				if(iHeightTable < Vocabulario_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS) {//Constantes.ISWING_TAMANIOMINIMO_TABLADATOS) {
					iHeightTable=Vocabulario_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS; //Constantes.ISWING_TAMANIOMINIMO_TABLADATOS;
				}
			}
			
		} else {
			if(iHeightTable > Vocabulario_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS_REL) { //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS) {
				//SI SE SELECCIONA MAXIMO TABLA SE AMPLIA A ALTO MAXIMO DE SCROLL, PARA QUE SCROLL NO SEA TAN PEQUE?O
				
				if(!this.jPanelParametrosReportes.chb_ConAltoMaximoTabla.isSelected()) {
					iHeightTable=Vocabulario_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS_REL; //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS;
				} else {
				
					iHeightTable=iHeightTableTotal + FuncionesSwing.getValorProporcion(iHeightTableTotal,30);
				}
			} else {
				if(iHeightTable < Vocabulario_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS_REL) {//Constantes.ISWING_TAMANIOMINIMO_TABLADATOS) {
					iHeightTable=Vocabulario_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS_REL; //Constantes.ISWING_TAMANIOMINIMO_TABLADATOS;
				}
			}
		}
	...
	//PARA REPORTES
	public static List<Vocabulario> TraerBeans(List<Vocabulario> vocabularios,ArrayList<Classe> classes)throws Exception {
		try {
			for(Vocabulario vocabulario:vocabularios) {
					
				if(!(Vocabulario_util.S_TIPO_REPORTE_EXTRA.equals(Constantes2.S_REPORTE_EXTRA_GROUP_GENERICO)
						  || Vocabulario_util.S_TIPO_REPORTE_EXTRA.equals(Constantes2.S_REPORTE_EXTRA_GROUP_TOTALES_GENERICO)) 
				   ) {
					
					vocabulario.setsDetalleGeneralEntityReporte(Vocabulario_util.getVocabularioDescripcion(vocabulario));
					...
					//vocabulario.setsDetalleGeneralEntityReporte(vocabulario.getsDetalleGeneralEntityReporte());
										
				}
				
				//vocabulariobeans.add(vocabulariobean);
			}
		} catch(Exception ex) {
			throw ex;
		}
	...
	/*
		public interface Vocabulario_windowI {	
