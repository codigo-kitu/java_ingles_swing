...
public class Accion_window extends JInternalFrameBase {	
	...
	public Accion_form_window jFrameDetalleForm;//_form;		
	...
	*/
	...
	public Accion_window() throws Exception {
	...
	public Accion_window(Boolean cargarRelaciones,PaginaTipo paginaTipo) throws Exception {
		super(paginaTipo);
        //super("Accion No " + (++openFrameCount),true, /*resizable*/true, /*closable*/true, /*maximizable*/true);//iconifiable
	  	try {
	...
	public Accion_window(Boolean conGuardarRelaciones,Boolean esGuardarRelacionado,Boolean cargarRelaciones,PaginaTipo paginaTipo) throws Exception {
		super(paginaTipo);
        //super("Accion No " + (++openFrameCount),true, /*resizable*/true, /*closable*/true, /*maximizable*/true);//iconifiable
	  	try {
	...
	//	this.jCheckBoxConGraficoDinamicoAccion = jCheckBoxConGraficoDinamico;
	//}	
	...
	*/
	...
	private void initialize(JDesktopPane jdesktopPane,Boolean conGuardarRelaciones,Boolean esGuardarRelacionado,Boolean cargarRelaciones,Usuario usuarioActual,ResumenUsuario resumenUsuarioActual,Modulo moduloActual,Opcion opcionActual,ParametroGeneralSg parametroGeneralSg,ParametroGeneralUsuario parametroGeneralUsuario,PaginaTipo paginaTipo) throws Exception {
		...
		//this.sTipoTamanioGeneral=Accion_window.S_TIPO_TAMANIO_GENERAL;
		//this.sTipoTamanioGeneralTabla=Accion_window.S_TIPO_TAMANIO_GENERAL_TABLA;
	...
	public void inicializarElementosBusquedas() {
		...
		FuncionesSwing.setLabelSize(lbl_id_opcionFK_IdOpcion,Constantes.I_SWING_ANCHO_CONTROL_LABEL + FuncionesSwing.getValorProporcion(Constantes.I_SWING_ANCHO_CONTROL_LABEL,-43),Constantes2.I_SWING_ALTO_CONTROL_LABEL);
	...
	public JScrollPane getJContentPane(int iWidth,int iHeight,JDesktopPane jDesktopPane,Boolean cargarRelaciones,Usuario usuarioActual,ResumenUsuario resumenUsuarioActual,Modulo moduloActual,Opcion opcionActual,ParametroGeneralSg parametroGeneralSg,ParametroGeneralUsuario parametroGeneralUsuario,PaginaTipo paginaTipo) throws Exception {	
		...
		if(this.conCargarFormDetalle) { //true) {
			//this.jFrameDetalle = new AccionDetalle_window(paginaTipo);//JInternalFrameBase("Accion DATOS");
			//_form
			this.jFrameDetalleForm = new Accion_form_window(jDesktopPane,this.accion_session.getConGuardarRelaciones(),this.accion_session.getEsGuardarRelacionado(),cargarRelaciones,usuarioActual,resumenUsuarioActual,moduloActual,opcionActual,parametroGeneralSg,parametroGeneralUsuario,paginaTipo);
		} else {
			//_form
			this.jFrameDetalleForm = null;//new Accion_form_window("MINIMO");
		}
		...
		this.gridaBagLayout= new GridBagLayout();//Accion
		...
		this.tableDatos.setToolTipText(sToolTipAccion);
		...
		if(!this.conCargarMinimo) {
			...
			//this.jFrameOrderBy=new OrderByJInternalFrame(S_TIPO_TAMANIO_GENERAL,this.jButtonAbrirOrderByAccion,false,this);
			
			//this.cargarOrderBy(false);
		} else {
			//NO CARGAR INICIALMENTE, EN BOTON AL ABRIR
			//this.jFrameOrderBy=new OrderByJInternalFrame(S_TIPO_TAMANIO_GENERAL,this.jButtonAbrirOrderByAccion,true,this);
		...
		*/
		...
		this.jPanelParametrosReportes = new JPanelParametrosReportes(this,"Accion",this.parametroGeneralUsuario.getid_tipo_fondo(),S_TIPO_TAMANIO_GENERAL,this.conCargarMinimo,false,false,true);//new JPanel();
		...
		this.jPanelPaginacion = new JPanelPaginacion(this,this.parametroGeneralUsuario.getid_tipo_fondo(),true,this.conCargarMinimo,this.accion_session.getEsGuardarRelacionado(),S_TIPO_TAMANIO_GENERAL,"Accion",PaginaTipo.PRINCIPAL,false,this.conFuncionalidadRelaciones,false);
		this.jPanelToolBar = new JPanelToolBar(this.parametroGeneralUsuario.getid_tipo_fondo(),"Accion",PaginaTipo.PRINCIPAL,this);
		...
		//FuncionesSwing.addImagenButtonGeneral(this.jButtonRecargarInformacion,"recargar_button","Recargar","Recargar"+" "+Accion_util.S_CLASS_WEB_TITULO + FuncionesSwing.getKeyMensaje("RECARGAR"));		
		
		
		
		
		this.btn_ProcesarInformacion = new JButton();
		FuncionesSwing.setButtonProp(this.btn_ProcesarInformacion,"Procesar","Procesar"+" "+Accion_util.S_CLASS_WEB_TITULO,false);
		...
		*/
		...
		//ESTA EN BEAN
		/*
		this.jTabbedPaneRelaciones.setMinimumSize(new Dimension(this.getWidth(),AccionBean.ALTO_TABPANE_RELACIONES  + FuncionesSwing.getValorProporcion(AccionBean.ALTO_TABPANE_RELACIONES,0)));
		this.jTabbedPaneRelaciones.setMaximumSize(new Dimension(this.getWidth(),AccionBean.ALTO_TABPANE_RELACIONES  + FuncionesSwing.getValorProporcion(AccionBean.ALTO_TABPANE_RELACIONES,0)));
		this.jTabbedPaneRelaciones.setPreferredSize(new Dimension(this.getWidth(),AccionBean.ALTO_TABPANE_RELACIONES  + FuncionesSwing.getValorProporcion(AccionBean.ALTO_TABPANE_RELACIONES,0)));
		*/
		...
		/*
			...
			this.jScrollPanelDatos.setLayout(gridaBagLayoutDatosAccion);
		...
		*/
		...
		this.jPanelAcciones.add(this.jButtonNuevoAccion, this.gridBagConstraints);
		*/
		int iPosXAccion=0;
		...
		pnl_FK_IdOpcion.setLayout(gridaBagLayoutFK_IdOpcionAccion);
		...
		jTabbedPaneBusquedas.addTab("1.-Por Opcion ", pnl_FK_IdOpcion);
...
if(this.parametroGeneralUsuario.getcon_botones_tool_bar() && !this.accion_session.getEsGuardarRelacionado()) {			
	...
	public void redimensionarTablaDatos(int iNumFilas,int iTamanioFilaTabla) {
		...
		if(this.conMaximoRelaciones 
			&& this.accion_session.getConGuardarRelaciones()
			) {
		...
		*/
		...
		if(iTamanioFilaTabla<=0) {
			iTamanioFilaTabla=this.tableDatos.getRowHeight();//Accion_util.ITAMANIOFILATABLA;
		}
		...
		if(!this.accion_session.getEsGuardarRelacionado()) {			
			if(iHeightTable > Accion_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS) { //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS) {
				//SI SE SELECCIONA MAXIMO TABLA SE AMPLIA A ALTO MAXIMO DE SCROLL, PARA QUE SCROLL NO SEA TAN PEQUE?O
				
				if(!this.jPanelParametrosReportes.chb_ConAltoMaximoTabla.isSelected()) {
					iHeightTable=Accion_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS; //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS;
				} else {
				
					iHeightTable=iHeightTableTotal + FuncionesSwing.getValorProporcion(iHeightTableTotal,30);
				}
			} else {
				if(iHeightTable < Accion_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS) {//Constantes.ISWING_TAMANIOMINIMO_TABLADATOS) {
					iHeightTable=Accion_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS; //Constantes.ISWING_TAMANIOMINIMO_TABLADATOS;
				}
			}
			
		} else {
			if(iHeightTable > Accion_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS_REL) { //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS) {
				//SI SE SELECCIONA MAXIMO TABLA SE AMPLIA A ALTO MAXIMO DE SCROLL, PARA QUE SCROLL NO SEA TAN PEQUE?O
				
				if(!this.jPanelParametrosReportes.chb_ConAltoMaximoTabla.isSelected()) {
					iHeightTable=Accion_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS_REL; //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS;
				} else {
				
					iHeightTable=iHeightTableTotal + FuncionesSwing.getValorProporcion(iHeightTableTotal,30);
				}
			} else {
				if(iHeightTable < Accion_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS_REL) {//Constantes.ISWING_TAMANIOMINIMO_TABLADATOS) {
					iHeightTable=Accion_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS_REL; //Constantes.ISWING_TAMANIOMINIMO_TABLADATOS;
				}
			}
		}
	...
	//PARA REPORTES
	public static List<Accion> TraerBeans(List<Accion> accions,ArrayList<Classe> classes)throws Exception {
		try {
			for(Accion accion:accions) {
					
				if(!(Accion_util.S_TIPO_REPORTE_EXTRA.equals(Constantes2.S_REPORTE_EXTRA_GROUP_GENERICO)
						  || Accion_util.S_TIPO_REPORTE_EXTRA.equals(Constantes2.S_REPORTE_EXTRA_GROUP_TOTALES_GENERICO)) 
				   ) {
					
					accion.setsDetalleGeneralEntityReporte(Accion_util.getAccionDescripcion(accion));
										
					accion.setestado_descripcion(Accion_util.getestadoDescripcion(accion));	
	...
	/*
		public interface Accion_windowI {	
