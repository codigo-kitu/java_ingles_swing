...
public class Campo_window extends JInternalFrameBase {	
	...
	public Campo_form_window jFrameDetalleForm;//_form;		
	...
	*/
	...
	public Campo_window() throws Exception {
	...
	public Campo_window(JDesktopPane jdesktopPane,Boolean conGuardarRelaciones,Boolean esGuardarRelacionado,Boolean cargarRelaciones,Usuario usuarioActual,ResumenUsuario resumenUsuarioActual,Modulo moduloActual,Opcion opcionActual,ParametroGeneralSg parametroGeneralSg,ParametroGeneralUsuario parametroGeneralUsuario,PaginaTipo paginaTipo) throws Exception {
	...
	public Campo_window(Boolean cargarRelaciones,PaginaTipo paginaTipo) throws Exception {
		super(paginaTipo);
        //super("Campo No " + (++openFrameCount),true, /*resizable*/true, /*closable*/true, /*maximizable*/true);//iconifiable
	  	try {
	...
	public Campo_window(Boolean conGuardarRelaciones,Boolean esGuardarRelacionado,Boolean cargarRelaciones,PaginaTipo paginaTipo) throws Exception {
		super(paginaTipo);
        //super("Campo No " + (++openFrameCount),true, /*resizable*/true, /*closable*/true, /*maximizable*/true);//iconifiable
	  	try {
	...
	//	this.jCheckBoxConGraficoDinamicoCampo = jCheckBoxConGraficoDinamico;
	//}	
	...
	*/
	...
	private void initialize(JDesktopPane jdesktopPane,Boolean conGuardarRelaciones,Boolean esGuardarRelacionado,Boolean cargarRelaciones,Usuario usuarioActual,ResumenUsuario resumenUsuarioActual,Modulo moduloActual,Opcion opcionActual,ParametroGeneralSg parametroGeneralSg,ParametroGeneralUsuario parametroGeneralUsuario,PaginaTipo paginaTipo) throws Exception {
		...
		//this.sTipoTamanioGeneral=Campo_window.S_TIPO_TAMANIO_GENERAL;
		//this.sTipoTamanioGeneralTabla=Campo_window.S_TIPO_TAMANIO_GENERAL_TABLA;
	...
	public JScrollPane getJContentPane(int iWidth,int iHeight,JDesktopPane jDesktopPane,Boolean cargarRelaciones,Usuario usuarioActual,ResumenUsuario resumenUsuarioActual,Modulo moduloActual,Opcion opcionActual,ParametroGeneralSg parametroGeneralSg,ParametroGeneralUsuario parametroGeneralUsuario,PaginaTipo paginaTipo) throws Exception {	
		...
		if(this.conCargarFormDetalle) { //true) {
			//this.jFrameDetalle = new CampoDetalle_window(paginaTipo);//JInternalFrameBase("Campo DATOS");
			//_form
			this.jFrameDetalleForm = new Campo_form_window(jDesktopPane,this.campo_session.getConGuardarRelaciones(),this.campo_session.getEsGuardarRelacionado(),cargarRelaciones,usuarioActual,resumenUsuarioActual,moduloActual,opcionActual,parametroGeneralSg,parametroGeneralUsuario,paginaTipo);
		} else {
			//_form
			this.jFrameDetalleForm = null;//new Campo_form_window("MINIMO");
		}
		...
		this.gridaBagLayout= new GridBagLayout();//Campo
		...
		this.tableDatos.setToolTipText(sToolTipCampo);
		...
		if(!this.conCargarMinimo) {
			...
			//this.jFrameOrderBy=new OrderByJInternalFrame(S_TIPO_TAMANIO_GENERAL,this.jButtonAbrirOrderByCampo,false,this);
			
			//this.cargarOrderBy(false);
		} else {
			//NO CARGAR INICIALMENTE, EN BOTON AL ABRIR
			//this.jFrameOrderBy=new OrderByJInternalFrame(S_TIPO_TAMANIO_GENERAL,this.jButtonAbrirOrderByCampo,true,this);
		...
		*/
		...
		this.jPanelParametrosReportes = new JPanelParametrosReportes(this,"Campo",this.parametroGeneralUsuario.getid_tipo_fondo(),S_TIPO_TAMANIO_GENERAL,this.conCargarMinimo,false,false,true);//new JPanel();
		...
		this.jPanelPaginacion = new JPanelPaginacion(this,this.parametroGeneralUsuario.getid_tipo_fondo(),true,this.conCargarMinimo,this.campo_session.getEsGuardarRelacionado(),S_TIPO_TAMANIO_GENERAL,"Campo",PaginaTipo.PRINCIPAL,false,this.conFuncionalidadRelaciones,false);
		this.jPanelToolBar = new JPanelToolBar(this.parametroGeneralUsuario.getid_tipo_fondo(),"Campo",PaginaTipo.PRINCIPAL,this);
		...
		//FuncionesSwing.addImagenButtonGeneral(this.jButtonRecargarInformacion,"recargar_button","Recargar","Recargar"+" "+Campo_util.S_CLASS_WEB_TITULO + FuncionesSwing.getKeyMensaje("RECARGAR"));		
		
		
		
		
		this.btn_ProcesarInformacion = new JButton();
		FuncionesSwing.setButtonProp(this.btn_ProcesarInformacion,"Procesar","Procesar"+" "+Campo_util.S_CLASS_WEB_TITULO,false);
		...
		*/
		...
		//ESTA EN BEAN
		/*
		this.jTabbedPaneRelaciones.setMinimumSize(new Dimension(this.getWidth(),CampoBean.ALTO_TABPANE_RELACIONES  + FuncionesSwing.getValorProporcion(CampoBean.ALTO_TABPANE_RELACIONES,0)));
		this.jTabbedPaneRelaciones.setMaximumSize(new Dimension(this.getWidth(),CampoBean.ALTO_TABPANE_RELACIONES  + FuncionesSwing.getValorProporcion(CampoBean.ALTO_TABPANE_RELACIONES,0)));
		this.jTabbedPaneRelaciones.setPreferredSize(new Dimension(this.getWidth(),CampoBean.ALTO_TABPANE_RELACIONES  + FuncionesSwing.getValorProporcion(CampoBean.ALTO_TABPANE_RELACIONES,0)));
		*/
		...
		/*
			...
			this.jScrollPanelDatos.setLayout(gridaBagLayoutDatosCampo);
		...
		*/
		...
		this.jPanelAcciones.add(this.jButtonNuevoCampo, this.gridBagConstraints);
		*/
		int iPosXAccion=0;
		...
		pnl_FK_IdOpcion.setLayout(gridaBagLayoutFK_IdOpcionCampo);
...
if(this.parametroGeneralUsuario.getcon_botones_tool_bar() && !this.campo_session.getEsGuardarRelacionado()) {			
	...
	public void redimensionarTablaDatos(int iNumFilas,int iTamanioFilaTabla) {
		...
		if(this.conMaximoRelaciones 
			&& this.campo_session.getConGuardarRelaciones()
			) {
		...
		*/
		...
		if(iTamanioFilaTabla<=0) {
			iTamanioFilaTabla=this.tableDatos.getRowHeight();//Campo_util.ITAMANIOFILATABLA;
		}
		...
		if(!this.campo_session.getEsGuardarRelacionado()) {			
			if(iHeightTable > Campo_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS) { //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS) {
				//SI SE SELECCIONA MAXIMO TABLA SE AMPLIA A ALTO MAXIMO DE SCROLL, PARA QUE SCROLL NO SEA TAN PEQUE?O
				
				if(!this.jPanelParametrosReportes.chb_ConAltoMaximoTabla.isSelected()) {
					iHeightTable=Campo_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS; //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS;
				} else {
				
					iHeightTable=iHeightTableTotal + FuncionesSwing.getValorProporcion(iHeightTableTotal,30);
				}
			} else {
				if(iHeightTable < Campo_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS) {//Constantes.ISWING_TAMANIOMINIMO_TABLADATOS) {
					iHeightTable=Campo_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS; //Constantes.ISWING_TAMANIOMINIMO_TABLADATOS;
				}
			}
			
		} else {
			if(iHeightTable > Campo_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS_REL) { //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS) {
				//SI SE SELECCIONA MAXIMO TABLA SE AMPLIA A ALTO MAXIMO DE SCROLL, PARA QUE SCROLL NO SEA TAN PEQUE?O
				
				if(!this.jPanelParametrosReportes.chb_ConAltoMaximoTabla.isSelected()) {
					iHeightTable=Campo_util.TAMANIO_ALTO_MAXIMO_TABLA_DATOS_REL; //Constantes.ISWING_TAMANIOMAXIMO_TABLADATOS;
				} else {
				
					iHeightTable=iHeightTableTotal + FuncionesSwing.getValorProporcion(iHeightTableTotal,30);
				}
			} else {
				if(iHeightTable < Campo_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS_REL) {//Constantes.ISWING_TAMANIOMINIMO_TABLADATOS) {
					iHeightTable=Campo_util.TAMANIO_ALTO_MINIMO_TABLA_DATOS_REL; //Constantes.ISWING_TAMANIOMINIMO_TABLADATOS;
				}
			}
		}
	...
	//PARA REPORTES
	public static List<Campo> TraerBeans(List<Campo> campos,ArrayList<Classe> classes)throws Exception {
		try {
			for(Campo campo:campos) {
					
				if(!(Campo_util.S_TIPO_REPORTE_EXTRA.equals(Constantes2.S_REPORTE_EXTRA_GROUP_GENERICO)
						  || Campo_util.S_TIPO_REPORTE_EXTRA.equals(Constantes2.S_REPORTE_EXTRA_GROUP_TOTALES_GENERICO)) 
				   ) {
					
					campo.setsDetalleGeneralEntityReporte(Campo_util.getCampoDescripcion(campo));
										
					campo.setestado_descripcion(Campo_util.getestadoDescripcion(campo));	
	...
	/*
		public interface Campo_windowI {	
