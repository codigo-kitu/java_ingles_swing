	...
	public IdTableCell(JInternalFrameBase jInternalFrameBase,Boolean esRelaciones,Boolean esEliminar,Boolean conEnabled,Integer iTotalRow,Boolean esRelacionado) {
		...
		if(!this.esEliminar) {
			if(!this.esRelaciones) {
				if(!this.jInternalFrameBase.esParaBusquedaFK) {
	...
	public Component getTableCellRendererComponent(JTable table,Object value,boolean isSelected,boolean hasFocus,final int row,final int column) {
		...
		*/
		...
		if(this.esEliminar) {
			...
			FuncionesSwing.setBoldButton(this.jButtonId, this.jInternalFrameBase.sTipoTamanioGeneral,true,true,this.jInternalFrameBase);
			//FuncionesSwing.setBoldComponent(this.jButtonId, this.jInternalFrameBase.sTipoTamanioGeneral,true,true,this.jInternalFrameBase);
		...
		//System.out.println(this.jInternalFrameBase.sTipoTamanioGeneral);
		
		this.jButtonId.setOpaque(true);
		
		//if(row!=(this.iTotalRow-1)) {
		if((this.jInternalFrameBase.conTotales && row != table.getRowCount()-1) || !this.jInternalFrameBase.conTotales) {
			this.jButtonId.setBackground(row % 2 == 0 ? FuncionesSwing.getColorTextFields(Constantes2.S_FONDO_CONTROL_COLOR) : Funciones2.getColorFilaTabla2());
		} else {
