...
//import com.bydan.ingles.contabilidad.business.entity.AsientoContable;
...
public class Funciones2 {	
	...
	public static Boolean verificarGetPermisosUsuarioOpcionClaseRelacionada(List<Opcion> opcionsFinal,String sPaginaActual) throws Exception {
		Boolean verificado=false;
		String sNombreClaseSolo="";
		
		for(Opcion opcion:opcionsFinal) {
			
			sNombreClaseSolo=opcion.getnombre_clase().replace("Mantenimiento", "").replace(".jsf", "");
			
			if(opcion.getnombre_clase().equals(sPaginaActual) || sNombreClaseSolo.equals(sPaginaActual)) {
				verificado=true;
				break;
			}
		}
